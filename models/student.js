...
module.exports = {
	...
	login: function(tablename,password,email,enrollment_no, callback) {
		...
		var query2 = 'select * from ?? where enrollment_no= ?';
		this.login2(tablename,password,enrollment_no,callback);
        console.log('----------------');
        con.query(query2,[tablename,enrollment_no], function(err, rows) {
        	if (err) {
				return console.log(err);
        	}
        	console.log(rows);
			// var cb=callback(null,rows);
			// console.log(cb);
        	callback(null, rows);
         })
	},

	login2: function(tablename, password, enrollment_no, callback) {
        var query  = 'update ?? set password = ? where enrollment_no= ?';
		// var query2 = 'select * from ?? where enrollment_no= ?';
        console.log('----------------');
        con.query(query,[tablename,password, enrollment_no], function(err, rows) {
        	if (err) {
				return console.log(err);
        	}
        	console.log(rows);

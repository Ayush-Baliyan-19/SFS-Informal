...
module.exports = {
      ...
      if(res3[0]['s_' + semester]=='1') {
        ...
        res.send(obj);
          ...
          student.login(tablename,password,email,enrollment_no, function(err, result) {
            ...
            console.log("ahuahuahuahuahuahu");
            console.log(result);
            if(result[0].email != request.email) {
  ...
  dashboard:function(req,res) {
    ...
    console.log(req.param("enrollment_no")+"req.paramenroll");
    var enrollment_no = req.param("enrollment_no");
...
//--------------------------feedback------------------------------//
  ...
  feedbackform : function(req,res) {
      ...
      if(req.session.student.course && req.session.student.stream && req.session.student.year && req.session.student.college_name) {
          ...
          console.log("courrrrrrse",course);
          console.log(stream);
          console.log(semester);
	  if(course=="M. TECH" && _.includes(['usict','usct','usbt'],college_name) && semester>8) semester-=8;
          student.getFeedbackInfo(tablename1, tablename2, tablename3, course, stream, semester, function(err, result) {
            if(err) {
              console.log(err);
              res.status(400);
            }
            console.log(result,"teachers for feedback");
            res.json(result)
          })
      } else {
        console.log("Not all fields set");
